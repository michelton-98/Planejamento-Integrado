...
function TooltipPersonalizado({ active, payload }) {
  ...
  return (
    <div className="rounded-md border border-gray-200 bg-white px-3 py-2 text-sm shadow-sm">
      <p className="font-medium text-navy">{item.payload.nome}</p>
      <p className="text-gray-600">
        <span className="font-semibold text-navy">{item.value}</span> pendência(s)
      </p>
    </div>
  )
}
          ...
          <BarChart data={data} layout="vertical" margin={{ top: 4, right: 32, bottom: 4, left: 4 }}>
            ...
            >
              ...
              <LabelList
                dataKey="total"
                position="right"
                style={{ fill: '#12263f', fontSize: 12, fontWeight: 600 }}
              />
            </Bar>
          </BarChart>
        </ResponsiveContainer>
